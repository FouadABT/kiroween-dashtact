  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3 p-4 border border-border rounded-lg bg-muted/30">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <Label htmlFor="show-trust-badges">Trust Badges</Label>
            <p className="text-xs text-muted-foreground">
              Show badge with key features or benefits
            </p>
          </div>
          <Switch
            id="show-trust-badges"
            checked={data.showTrustBadges || false}
            onCheckedChange={(checked) => handleChange('showTrustBadges', checked)}
          />
        </div>

        {data.showTrustBadges && (
          <div className="space-y-2 pt-2">
            <Label>Badge Items</Label>
            <div className="space-y-2">
              {(data.trustBadges || []).map((badge, index) => (
                <div key={index} className="flex gap-2">
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() => {
                      const newBadges = (data.trustBadges || []).filter((_, i) => i !== index);
                      handleChange('trustBadges', newBadges);
                    }}
                  >
                    <X className="h-4 w-4" />
            ...
            <p className="text-xs text-muted-foreground">
              Items will be separated by bullets (•) in the badge
            </p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        <Select
          value={data.height}
          onValueChange={(value: unknown) => handleChange('height', value)}
        >
