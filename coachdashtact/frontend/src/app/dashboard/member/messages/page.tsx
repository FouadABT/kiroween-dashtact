...
export default function MemberMessagesPage() {
  ...
  }, [messaging, urlConversationId]);

  const handleSelectConversation = (conversationId: string) => {
    messaging.selectConversation(conversationId);
    router.push(`/dashboard/member/messages?conversation=${conversationId}`);
  };
      ...
      <PageHeader
        ...
        breadcrumbProps={{
          customItems: [
            ...
            { label: 'Messages', href: '/dashboard/member/messages' },
          ],
        }}
      />
          ...
          <ScrollArea className="flex-1">
            {messaging.isLoading ? (
              ...
              <div className="p-4 text-center text-muted-foreground">
                ...
                <p className="text-sm mt-2">Start by messaging your coach</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start gap-3">
                        <Avatar className="h-10 w-10 flex-shrink-0">
                          ...
                          <AvatarFallback className="bg-muted text-muted-foreground">
                            {otherParticipant?.user?.name?.[0]?.toUpperCase() || 'C'}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            <p className="font-medium text-foreground truncate">
                              {conversation.name || otherParticipant?.user?.name || 'Coach'}
                            </p>
              ...
              <div className="p-4 border-b border-border flex items-center gap-3">
                <Button
                  ...
                  onClick={() => {
                    ...
                    router.push('/dashboard/member/messages');
                  }}
                  className="md:hidden"
                >
                            ...
                            {showAvatar ? (
                              <Avatar className="h-8 w-8">
                                ...
                                <AvatarFallback className="text-xs bg-muted text-muted-foreground">
                                  {isOwn ? 'You' : message.sender?.name?.[0]?.toUpperCase() || 'C'}
                                </AvatarFallback>
                              </Avatar>
                            ) : (
                          ...
                          <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} max-w-[70%]`}>
                            {showAvatar && (
                              <span className="text-xs text-muted-foreground mb-1 px-1">
                                {isOwn ? 'You' : message.sender?.name || 'Coach'}
                              </span>
                            )}
                            <div
