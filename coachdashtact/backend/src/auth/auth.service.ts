...
export class AuthService {
  ...
  /**
   * Register a new member with profile creation
   * @param registerMemberDto Member registration data including coach selection
   * @returns Authentication response with user and tokens
   */
  async registerMember(registerMemberDto: { email: string; password: string; name?: string; coachId?: string; goals?: string; healthInfo?: string }): Promise<AuthResponse> {
    const { email, password, name, coachId, goals, healthInfo } = registerMemberDto;
    ...
    try {
      ...
      if (!memberRole) {
        ...
        throw new Error('Member role not found. Please run database seed.');
      }
      ...
      if (coachId) {
        ...
        if (!coachProfile) {
          throw new BadRequestException('Selected coach not found');
        }

        if (!coachProfile.isAcceptingMembers) {
          throw new BadRequestException('This coach is not currently accepting new members');
        }
        ...
        if (currentMemberCount >= coachProfile.maxMembers) {
          throw new BadRequestException('Coach has reached maximum member capacity');
        }
      }
      ...
      try {
        if (this.notificationsService) {
          const welcomeNotification = createSystemNotification({
            ...
            message: `Hi ${name || 'there'}! Welcome to the coaching platform. ${coachId ? 'Your coach will be in touch soon.' : 'An administrator will assign you a coach shortly.'}`,
            actionUrl: '/member',
      ...
      // Create conversation with coach if assigned
      if (coachId) {
        try {
          // Note: This would require MessagingService to be injected
          // For now, we'll skip this and let the MembersService handle it
          this.logger.log(`Member ${result.user.id} assigned to coach ${coachId}`);
        } catch (error) {
          this.logger.warn('Failed to create coach conversation:', error);
        }
      }
      ...
      // Build user profile
      const userProfile = this.buildUserProfile(result.user);
